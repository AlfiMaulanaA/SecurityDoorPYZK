    ...
    <div>
        ...
        <div class="content-style-primary">
            <div class="d-flex justify-content-between align-items-center">
                <h5>Data Shape Layout {{ layoutGroupId }}</h5>
                <span>
                    <span class="me-3" :class="mqttConnected ? 'text-success' : 'text-danger'">
                        {{ mqttConnected ? "Connected" : "Disconnected" }}
                    </span>
                    <router-link to="/layout-groups" class="btn btn-sm btn-secondary me-2">Go To List</router-link>
                    <router-link :to="`/layout-groups/${layoutGroupId}`" class="btn btn-sm btn-secondary me-2">
                        Go To Canvas
                    </router-link>
            ...
            <div>
                ...
                <button class="btn btn-sm btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#templateModal">
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="templateModalLabel">Select Shape Template</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="d-flex flex-wrap gap-2">
                                    <button v-for="template in templateShape" :key="template.id"
                                        class="btn btn-sm btn-outline-primary" @click="addShapeFromTemplate(template)">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in paginatedItems" :key="index">
                            ...
                            <td>{{ item.fillColor ? 'Yes' : 'No' }} ({{ item.color }})</td>
                            <td>{{ item.border ? 'Yes' : 'No' }} ({{ item.borderColor }}) {{ item.borderWidth }}</td>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary"
                            @click="() => { console.log('Shape Payload Before Save:', newItem.value); saveNewItem(); }"
...
const addShapeFromTemplate = async (template) => {
    ...
    if (result.isConfirmed) {
        const newShape = { ...template, layoutGroupId };
        delete newShape.id;
        newShape.name;

        try {
            await axios.post(`http://localhost:5072/shapes`, newShape);
...
const saveNewItem = async () => {
    console.log("Shape Payload:", newItem.value); // Log sebelum data dikirim

    try {
        const valueAsString =
            newItem.value.value !== undefined ? String(newItem.value.value) : "";

        const response = await axios.post(
            `http://localhost:5072/shapes`,
            {
        ...
        await fetchShapes();
        resetNewItem(); // Reset form setelah berhasil disimpan
    } catch (error) {
...
const updateSelectedItem = async () => {
    try {
        ...
        await axios.put(`${`http://localhost:5072/shapes`}/${selectedItem.value.id}`, payload);
